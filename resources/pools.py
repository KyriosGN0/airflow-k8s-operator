...
def create_pool(meta, spec, namespace, logger, body, **kwargs):
    ...
    try:
        ...
        RESOURCE_RECONCILIATION_DURATION.labels(resource_type='pool', operation='create').observe(duration)
        RESOURCE_OPERATIONS.labels(resource_type='pool', operation='create', status='success').inc()
        ...
        duration = time.time() - start_time
        RESOURCE_RECONCILIATION_DURATION.labels(resource_type='pool', operation='create').observe(duration)
        RESOURCE_OPERATIONS.labels(resource_type='pool', operation='create', status='failure').inc()
...
def delete_pool(meta, spec, namespace, logger, body, **kwargs):
    ...
    try:
        ...
        duration = time.time() - start_time
        RESOURCE_RECONCILIATION_DURATION.labels(resource_type='pool', operation='delete').observe(duration)
        RESOURCE_OPERATIONS.labels(resource_type='pool', operation='delete', status='success').inc()
        ...
        RESOURCE_OPERATIONS.labels(resource_type='pool', operation='delete', status='failure').inc()
        RECONCILIATION_FAILURES.labels(resource_type='pool').inc()
...
def update_pool(meta, spec, namespace, logger, body, **kwargs):
    ...
    try:
        ...
        duration = time.time() - start_time
        RESOURCE_RECONCILIATION_DURATION.labels(resource_type='pool', operation='update').observe(duration)
        RESOURCE_OPERATIONS.labels(resource_type='pool', operation='update', status='success').inc()
        ...
        duration = time.time() - start_time
        RESOURCE_RECONCILIATION_DURATION.labels(resource_type='pool', operation='update').observe(duration)
        RESOURCE_OPERATIONS.labels(resource_type='pool', operation='update', status='failure').inc()
        RECONCILIATION_FAILURES.labels(resource_type='pool').inc()
