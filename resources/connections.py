...
def create_connection(meta, spec, namespace, logger, body, **kwargs):
    ...
    try:
        ...
        RESOURCE_RECONCILIATION_DURATION.labels(resource_type='connection', operation='create').observe(duration)
        RESOURCE_OPERATIONS.labels(resource_type='connection', operation='create', status='success').inc()
        ...
        duration = time.time() - start_time
        RESOURCE_RECONCILIATION_DURATION.labels(resource_type='connection', operation='create').observe(duration)
        RESOURCE_OPERATIONS.labels(resource_type='connection', operation='create', status='failure').inc()
...
def delete_connection(meta, spec, namespace, logger, body, **kwargs):
    connection_id = meta.get("name")
    var_conn_type = spec.get("connType", "unknown")
    ...
    try:
        ...
        duration = time.time() - start_time
        RESOURCE_RECONCILIATION_DURATION.labels(resource_type='connection', operation='delete').observe(duration)
        RESOURCE_OPERATIONS.labels(resource_type='connection', operation='delete', status='success').inc()
        ...
        RESOURCE_OPERATIONS.labels(resource_type='connection', operation='delete', status='failure').inc()
        RECONCILIATION_FAILURES.labels(resource_type='connection').inc()
...
def update_connection(meta, spec, namespace, logger, body, **kwargs):
    ...
    try:
        ...
        duration = time.time() - start_time
        RESOURCE_RECONCILIATION_DURATION.labels(resource_type='connection', operation='update').observe(duration)
        RESOURCE_OPERATIONS.labels(resource_type='connection', operation='update', status='success').inc()

        return {"message": f"Connection {connection_id} updated successfully."}
    except Exception as e:
        duration = time.time() - start_time
        RESOURCE_RECONCILIATION_DURATION.labels(resource_type='connection', operation='update').observe(duration)
        RESOURCE_OPERATIONS.labels(resource_type='connection', operation='update', status='failure').inc()
        RECONCILIATION_FAILURES.labels(resource_type='connection').inc()
